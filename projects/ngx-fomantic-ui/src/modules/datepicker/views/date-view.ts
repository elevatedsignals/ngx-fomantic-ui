...
export class CalendarRangeDateService extends CalendarRangeService {
  public calcStart(start: Date): Date {
    const monthStart = DateUtil.startOf(
      DatePrecision.Month,
      DateUtil.clone(start)
    );
    monthStart.setDate(
      (1 - monthStart.getDay() + this.service.firstDayOfWeek - 7) % 7
    );
    return monthStart;
  }
...
export class FuiCalendarDateView extends CalendarView {
  constructor(renderer: Renderer2) {
    super(
      ...
      new CalendarRangeDateService(DatePrecision.Month, 6, 7)
    );
  }
  ...
  public get date(): string {
    return new DateParser(
      this.service.localeValues.formats.month,
      this.service.localeValues
